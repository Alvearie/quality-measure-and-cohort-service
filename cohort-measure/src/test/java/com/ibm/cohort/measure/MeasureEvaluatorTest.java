...
import com.ibm.cohort.cql.data.R4FhirModelResolverFactory;
...
public class MeasureEvaluatorTest extends MeasureTestBase {
	...
	public void elm_in_initial_population___cohort_evaluated_correctly() throws Exception {
		...
		WireMock.verify(1, WireMock.getRequestedFor(WireMock.urlEqualTo("/Patient/123?_format=json")));

		// These ensure that the cache is working
		WireMock.verify(1, WireMock.getRequestedFor(WireMock.urlMatching("/Library\\?url=http.*&_format=json")));
		WireMock.verify(1, WireMock.getRequestedFor(WireMock.urlEqualTo("/Library?name%3Aexact=" + library.getName() + "&version=1.0.0&_format=json")));
	}
	...
	public void id_based_library_link___successfully_loaded_and_evaluated() throws Exception {
		...
		WireMock.verify(1, WireMock.getRequestedFor(WireMock.urlEqualTo("/Library/" + library.getId() + "?_format=json")));
	}
	...
	public void measure_patient_list_report_generated() throws Exception {
		...
		MatcherAssert.assertThat(actualPatientIds, Matchers.containsInAnyOrder(patient1.getId(), patient2.getId()));
	}
