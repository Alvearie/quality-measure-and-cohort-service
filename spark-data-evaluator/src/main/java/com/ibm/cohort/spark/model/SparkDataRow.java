...
/**
 * Implements the DataRow interface using the data provided by
 * an Apache Spark Row object. DataType conversion is supplied for
 * all the Spark SQL data types except those of structured data - 
 * map, list, row - and binary data that doesn't have an equivalent
 * for CQL.
 * 
 * The CQL language has a rich set of functionality around dealing
 * with coded data, but there is no corresponding type in the Spark
 * SQL datatypes. In order to support conversion of a column in the 
 * Spark Row to a CQL System.Code object, the conversion logic supports
 * Spark column metadata. If a column contains the boolean value true
 * under the key "isCodeCol", then the column is assumed to be a String value
 * and will be automatically converted to a CQL code where System.Code.code is
 * set to the column value. Optionally, the metadata may contain systemCol
 * and displayCol metadata that indicate other columns in the row that
 * contain the System.Code.system and System.Code.display field data
 * that corresponds with the tagged column.
 * 
 * Conversion of other CQL-native types, such as interval and ratio, are
 * left to the CQL author to handle. 
 */
public class SparkDataRow implements DataRow {
    ...
    public Object getValue(String fieldName) {
    	...
    	if( sparkVal != null ) {
        	...
        	/**
        	 * The following logic uses column metadata fields provided in the Spark schema to identify
        	 * columns
        	 */
    ...
    public Set<String> getFieldNames() {
        return Arrays.stream(sparkRow.schema().fields())
