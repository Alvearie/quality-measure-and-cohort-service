 ...
 * produced by the Spark SQL Row API into the CQL typesystem. The general
 * approach is to pass through raw Java types where possible, convert whole
 * numbers to Integer, and convert real numbers into java.math.BigDecimal. Dates
 * are converted into the CQL engine runtime types. Binary and compound data
 * structures (map, list, row) are unsupported at this time.
 */
