...
class Test(object):
    ...
    def execute(self, params, library, version,  targets, output, source, expressions, measureServer):
        ...
        o = output.split('\n')
        callDetails = ["java", "-Xms1G", "-Xmx1G", "-Djavax.net.ssl.trustStore="+os.environ["TRUSTSTORE"], "-Djavax.net.ssl.trustStorePassword="+os.environ["TRUSTSTORE_PASSWORD"], "-Djavax.net.ssl.trustStoreType="+os.environ["TRUSTSTORE_TYPE"], "-jar", jar, "cohort-cli", "-f", libraries, "-l", library]
