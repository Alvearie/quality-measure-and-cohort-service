...
In client-server mode, evaluation of clinical quality language (CQL) queries is done in a process that retrieves data from a remote FHIR server that supports version R4 of the HL7 FHIR RESTful API. Older releases of FHIR are not supported at this time. This mode of operation provides the most flexibility for deployments while sacrificing some runtime performance for that flexibility. Users needing additional performance should consider the [server-only](user-guide/server-only-guide.md) and [big-data](spark-user-guide.md) modes of operation as alternatives to client-server mode.
